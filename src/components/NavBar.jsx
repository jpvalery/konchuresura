...
const sections = [
	{
		...
		title: (
			<>
				<span className="hidden lg:inline">Game Components</span>
				<span className="lg:hidden">Components</span>
			</>
		),
	},
...
export function NavBar() {
	...
	return (
		<div ref={navBarRef} className="sticky top-0 z-50">
						...
						>
							{!open && (
								<>
									<span
										aria-hidden="true"
										className="font-mono text-sm text-emerald-800"
									>
			...
			<div className="hidden sm:flex sm:h-32 sm:justify-center sm:border-t-2 sm:border-b-2 sm:border-stone-200 sm:bg-white/95 sm:[@supports(backdrop-filter:blur(0))]:bg-white/80 sm:[@supports(backdrop-filter:blur(0))]:backdrop-blur-sm">
				<ol
					role="list"
					className="mb-[-2px] grid auto-cols-[minmax(0,15rem)] grid-flow-col text-base font-medium text-stone-900 [counter-reset:section]"
				>
					{sections.map((section, sectionIndex) => (
							...
							<a
								...
								className={clsx(
									...
									sectionIndex === activeIndex
										? 'border-emerald-800 bg-emerald-50 text-emerald-800 before:text-emerald-800'
										: 'border-transparent before:text-stone-500 hover:bg-emerald-50/40 hover:before:text-stone-900',
								)}
							>
								{section.title}
							</a>
